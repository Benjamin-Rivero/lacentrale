package fr.hb.lacentrale.configuration;



import fr.hb.lacentrale.entity.*;
import fr.hb.lacentrale.entity.embeddable.UserListingId;
import fr.hb.lacentrale.repository.*;
        ...
        for (User user : users) {
            for (int i = 0; i < random.nextInt(2, 8); i++) {
                ...
                favorite.setCreatedAt(generateRandomDate(
                        Instant.now().minusSeconds(999999999)));
                UserListingId userListingId = new UserListingId(user.getUuid(),listing.getUuid());
